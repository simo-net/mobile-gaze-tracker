...
def keypoints_adder(files: list, p: str,
    ...
    if len(eye_errors):
        ...
        issue_file = os.path.join('/'.join(img_file.split('/')[:-3]), f'issue_files.txt')
        with open(issue_file, 'w') as f:
            f.writelines('\n'.join(eye_errors))
        print(f"There were {len(eye_errors)} out-of-image or out-of-eye detected eye-landmarks: keeping the old "
              f"eye-landmarks from metadata for those.\nIf you want to check them out: look at the file {issue_file}")
    if no_face:
        print(f"In total {no_face} samples were deleted since no face was detected and metadata had invalid face info.")
    else:
        print("All images were kept and eye-landmarks were successfully updated!")
    return eye_errors
...
#
#
# def add_keypoints_multiprocess(in_dir: str, p: str,
#                                workers: int = 2):
...
#         proc.join()
#
# def add_keypoints_different_shape_multiprocess(in_dir: str, p: str,
#                                                workers: int = 2):
#     from multiprocessing import Process
#     procs = []
#     files = sorted(glob(os.path.join(in_dir, "*", "images", "*.jpg")))
