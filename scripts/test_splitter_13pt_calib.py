...
Input should be root of folder containing dataset converted using dataset_converter_*.py and root directory of
completely unzipped GazeCapture dataset. 
'''
...
def split_dataset(root, folders, data_root, new_root):
    ...
    for i in tqdm(folders):
        ...
        calib_pts_nine = np.array([[un_pts.min(axis=0)[0], un_pts.min(axis=0)[1]],
                                   ...
                                   [(un_pts.max(axis=0)[0] + un_pts.min(axis=0)[0]) / 2,
                                    (un_pts.max(axis=0)[1] + un_pts.min(axis=0)[1]) / 2],
        ...
        calib_pts_thirteen = np.array(
            [[(calib_pts_nine[0][0] + calib_pts_nine[8][0]) / 2, (calib_pts_nine[0][1] + calib_pts_nine[6][1]) / 2],
